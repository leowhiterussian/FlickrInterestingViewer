...
import com.tomdignan.remoteimage.RemoteImageLoaderHandler.RemoteImageLoaderViewAdapter;
...
public class RemoteImageLoader
{
    ...
    // seems more like the size we're using!
    ...
    public RemoteImageLoader(Context context, boolean createCache)
    {
        ...
        if(createCache)
        {
            ...
            imageCache.enableDiskCache(context.getApplicationContext(),
                                       ImageCache.DISK_CACHE_INTERNAL);
        }
    ...
    public void loadImage(Drawable dummyDrawable, RemoteImageLoaderHandler handler)
    {
        ...
        if(view != null)
        {
            ...
            else
            {
                if(dummyDrawable != null)
                {
                    // Set the dummy image while waiting for the actual image to be loaded.
                    remoteImageLoaderViewAdapter.setDummyDrawableForView(dummyDrawable);
                }
        ...
        // We need to put this on a thread no matter what, (Well, we really don't,
        // but this is way simpler for the time being)
        // since the strategy for dealing with an OOM is to sleep and we cannot do that
        // on the main thread. This is just simpler, especially since OOM can easily
        // occur when the bitmap is being decoded.

        // Only do this if we appear to have enough memory. Otherwise we can get an OOM
        // exception when the bitmap is decoded.
        if(MemoryHelper.isEnoughMemoryLeft()
            && imageCache != null && imageCache.containsKeyInMemory
            (imageUrl))
        {
            try
            {
            ...
            {
                // If we OOM here, then try loading from a thread, since we
                // can sleep there and wait for the GC to clean things up.
                doLoaderJob(imageUrl, imageLoaderHandler, imageCache, numRetries,
                            defaultBufferSize);
            }
        }
        else
    ...
    /** Spawns a thread to load the image. */
    private void doLoaderJob(String imageUrl, RemoteImageLoaderHandler handler,
                             ImageCache cache, int numRetries, int defaultBufferSize)
    {
